...
LORs %>% mutate(binning = factor(binning, levels = c("bin_width = 1", "bin_width = 10"))) %>% ggplot(., aes(x = Lag, y = LORs))+
  ...
  geom_ribbon(aes(ymin=L_95_CI, ymax=U_95_CI, color = binning, fill = binning), alpha=0.3)+
...
plot_mod <- ggplot(LORs, aes(x = Lag, y = LORs))+
  ...
  geom_ribbon(aes(ymin=L_95_CI, ymax=U_95_CI, color = Method, fill = Method), alpha=0.3)+
  geom_hline(yintercept = 0) +
...
```{r ex11, eval=FALSE, include=FALSE}
...
```{r ex14, fig.cap="Green: EVariogram; Red: lorelogram::lorelogram"}
ggplot()+
