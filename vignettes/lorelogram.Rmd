...
GrayFox_Hour_long <- GrayFox_Hour_long %>% filter(!is.na(y)) # remove NAs in detection/nondetection
dim(GrayFox_Hour_long) # new dataset dimensions
...
The `EVariogram` function in the __CompRandFld__ package
```{r ex7}
library(CompRandFld) #EVariogram
 

tic()
lor_Evar <- EVariogram(data = GrayFox_Hour_long$y, coordx = GrayFox_Hour_long$time, coordy = rep(1, nrow(GrayFox_Hour_long)), maxdist = 30, type = "lorelogram", numbins = 30) 
toc() #183.58 sec elapsed


par(mfrow=c(2,1))
plot(lor_Evar$centers, lor_Evar$variograms, "l", ylim = c(-1,5), col = "green")
lines(lor_lor$Lag, lor_lor$LORs, ylim = c(-1,5), col = "red")
lines(lor_lor$Lag, lor_lor$U_95_CI, lty = 2, col = "red")
lines(lor_lor$Lag, lor_lor$L_95_CI, lty = 2, col = "red")
dev.off()
